 ...
 *
 *  As d_plus_clover_PRECISION was initially a CPU function, eta and phi are pointers to CPU arrays.
 *  For the CUDA version the vectors operations on eta and phi are preformed on the GPU, so there is
 *  a need to copy these vectors.
 * 
 *  In the future this function may become obsolete if FGMRES (and possibly other methods)
 *  support handling GPU vectors x and w instead, which would improve performance.
 * 
