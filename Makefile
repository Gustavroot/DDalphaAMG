...
dd_alpha_amg : $(OBJ) $(OBJ_CUDA)
	$(NVCC) $(NVCC_LINK_FLAGS) $(LIMEH) -o $@ $(OBJ) $(OBJ_CUDA) $(LIMELIB)
else
dd_alpha_amg : $(OBJ)
	$(CC) $(LINK_FLAGS) $(LIMEH) -o $@ $(OBJ) $(LIMELIB)
endif

ifeq ($(CUDA_ENABLER),yes)
dd_alpha_amg_db : $(OBJDB) $(OBJ_CUDADB)
	$(NVCC) -g $(NVCC_LINK_FLAGS) $(LIMEH) -o $@ $(OBJDB) $(OBJ_CUDADB) $(LIMELIB)
else
dd_alpha_amg_db : $(OBJDB)
	$(CC) -g $(LINK_FLAGS) $(LIMEH) -o $@ $(OBJDB) $(LIMELIB)
endif
...
$(BUILDDIR)/%.o: $(GSRCDIR)/%.c $(GHEA)
	@mkdir -p $(@D)
	$(CC) $(COMPILE_FLAGS) $(OPT_VERSION_FLAGS) $(LIMEH) -c $< -o $@ -lm

$(BUILDDIR)/%_db.o: $(GSRCDIR)/%.c $(GHEA)
	@mkdir -p $(@D)
	$(CC) -g $(COMPILE_FLAGS) $(DEBUG_VERSION_FLAGS) $(LIMEH) -DDEBUG -c $< -o $@
